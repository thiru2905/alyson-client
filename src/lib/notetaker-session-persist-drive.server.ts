...
function sessionRecallCheckIntervalMs(): number {
  // Default 5 min — cron is ~5 min; avoid Retrieve Bot on every bot every pass.
  const n = Number(process.env.NOTETAKER_SESSION_RECALL_CHECK_MS ?? String(5 * 60_000));
  return Number.isFinite(n) && n >= 30_000 ? Math.min(Math.floor(n), 15 * 60_000) : 5 * 60_000;
}
...
export async function driveSessionPersistToS3(
  ...
  if (!payload?.lines?.length) {
    ...
    if (botIndex?.cronFinalized || botIndex?.recallCallEndedAt) {
      // Already ended in S3 — backfill without a fresh Retrieve Bot when possible.
      if (botIndex.recallCallEndedAt && shouldFetchRecallForSession(id)) {
        return tryRecallBackfillForEndedBot(id);
      }
      return payload ? "empty" : "unavailable";
    }
    // Throttle: do not Retrieve Bot for every empty session on every cron/UI pass.
    if (!shouldFetchRecallForSession(id)) return payload ? "empty" : "unavailable";
  ...
  // Do not eagerly GET /bot/{id} here — inferMeetingEnded already throttles Recall checks.
  const { ended, recallCallEndedAt } = await inferMeetingEnded(session, lines, id, {
    lifecycle,
    botIndex,
    allowRecallFetch: !skipRecall && !botIndex?.cronFinalized && !botIndex?.recallCallEndedAt,
  });

  if (ended && !skipRecall && !botIndex?.cronFinalized) {
    // Merge uses Retrieve Bot (cached when inferMeetingEnded just fetched).
    // Throttle when end was inferred without a fresh lifecycle.
    if (recallCallEndedAt || shouldFetchRecallForSession(id)) {
  ...
  try {
    if (ended) {
      ...
      // Pipeline: meeting ended + idle >=15m → generate notes → SES auto-email.
      try {
    ...
    // Live meetings: persist transcript + advance idle clock only.
    // Notes + SES wait until the meeting has ended (see maybeAutoSendMeetingNotesEmail).
    await patchBotIndexCronStability(id, upstreamHash, {
      callEnded: false,
    }).catch(() => {});
    return "unchanged";
